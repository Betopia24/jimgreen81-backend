...
const formulaSchema = z.object({
  ...
  applicableIonicStrength: z
    .string()
    .min(1, "applicableIonicStrength is required"),
  formulaForInhibitionPerformance: z
    .string()
    .min(1, "formulaForInhibitionPerformance is required"),
});
...
const createRawMaterialSchema = z.object({
  ...
  manufacturerProductName: z
    .string()
    .min(1, "Manufacturer product name is required"),
  activeComponentName: z.string().min(1, "Active component name is required"),
  activePercentage: z.number().min(0).max(100),
  activePercentageChemicalFormula: z
    .string()
    .min(1, "Active percentage chemical formula is required"),
